...
describe("TKAI", function () {
    ...
    describe("Deployment", function () {
        ...
        it("Transfer 1M to address", async function () {
            ...
            expect(await tkai.balanceOf(STAGING_ACCOUNTS_ADDRESSES[1])).to.equal(1_000_000_000_000);            
        });

        it("Alloance 1M to address", async function () {
            const { tkai, owner } = await loadFixture(deployContract);
            await tkai.approve(STAGING_ACCOUNTS_ADDRESSES[1], 1_000_000_000_000)
            expect(await tkai.allowance(STAGING_ACCOUNTS_ADDRESSES[0], STAGING_ACCOUNTS_ADDRESSES[1])).to.equal(1_000_000_000_000);            
        });

        it("Transfer Event", async function () {
            const { tkai, owner } = await loadFixture(deployContract);
            await tkai.transfer(STAGING_ACCOUNTS_ADDRESSES[1], 1_000_000_000_000)
            await expect(tkai.transfer(STAGING_ACCOUNTS_ADDRESSES[1], 1_000_000_000_000))
            .to.emit(tkai, "Transfer")
            .withArgs(STAGING_ACCOUNTS_ADDRESSES[0], STAGING_ACCOUNTS_ADDRESSES[1], 1_000_000_000_000); // We accept any value as `when` arg            
        });
    })
})